...
function run_prepare {
    ...
    #[ -d ${BUILD_TOPDIR} ] && echo "${BUILD_DIR} already checked out (ok)" || make hretup-shr-core 2>&1
    ...
    if [ ! -d ${BUILD_TOPDIR}/buildhistory/ ] ; then
        cd ${BUILD_TOPDIR}/
        git clone git@github.com:akuster/jenkins-buildhistory.git
        cd buildhistory;
    ...
    echo 'BUILDHISTORY_COMMIT_AUTHOR ?= "Armin Kuster <akuster808@gmail.com>"' >> ${BUILD_TOPDIR}/conf/local.conf
...
# to fix fsoaudiod, alsa-state conflict in image-all
VIRTUAL-RUNTIME_alsa-state = "fsoaudiod"
# to fix apm, fso-apm conflict in image-all
...
#ERR_REPORT_PORT = "80"
#ERR_REPORT_USERNAME = "Armin Kuster"
#ERR_REPORT_EMAIL = "akuster808@gmail.com"
